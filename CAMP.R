###############################################################################
## CMap/LINCS 2020 数据处理流程 - GSVA-based NuS analysis
## Author: CJX
## Final workflow:
...
cat(sprintf("原始表达矩阵维度: %d 基因 × %d 样本\n",
            nrow(expr_raw), ncol(expr_raw)))
cat(sprintf("表达值范围: [%.2f, %.2f]\n",
...
cat(sprintf("筛选后表达矩阵维度: %d 基因 × %d 样本\n",
...
cat("Rdata中包含对象: ", paste(loaded_obj_names, collapse = ", "), "\n")

if (!"geneSets_final" %in% loaded_obj_names) {
  stop("加载失败：Rdata文件中未找到对象 'geneSets_final'")
}

if (is.null(geneSets_final$NuStress_UP) || is.null(geneSets_final$NuStress_DOWN)) {
  stop("加载失败：'geneSets_final' 中未找到 NuStress_UP 或 NuStress_DOWN")
}
...
###############################################################################
cat("正在进行NuS与normality score相关性分析...\n")
...
cat(sprintf("1. 原始表达矩阵: %d 基因 × %d 样本\n", nrow(expr_raw), ncol(expr_raw)))
cat(sprintf("2. 筛选后表达矩阵: %d 基因 × %d 样本\n", nrow(expr_filtered), ncol(expr_filtered)))
cat(sprintf("3. NuStress_UP 命中基因数: %d\n", length(geneSets_use$NuStress_UP)))
cat(sprintf("4. NuStress_DOWN 命中基因数: %d\n", length(geneSets_use$NuStress_DOWN)))
cat(sprintf("5. 获得NuS评分样本数: %d\n", nrow(nus_score_df)))
cat(sprintf("6. 唯一药物数: %d\n", nrow(drug_summary)))
cat(sprintf("7. 与normality score交集药物数: %d\n", nrow(merged_df)))
cat(sprintf("8. NuS score范围: [%.3f, %.3f]\n",
            min(drug_summary$mean_score, na.rm = TRUE),
            max(drug_summary$mean_score, na.rm = TRUE)))
cat("9. 输出图形: figure1-5.pdf\n")
cat("10. 输出表格: drug_summary_GSVA_NuS_score.csv, NuS_normality_intersection.csv\n")
cat(paste0(strrep("=", 70), "\n"))
