# Step5_治疗前后及原发转移比较 + untreated DE/GO（最终版）
# ======================================================================
# 1. 创建环境
# ======================================================================
getwd()
...
# ------------------------------
# 3.1 从 *_data.rda 中安全读取数据框
# 兼容你前面保存时对象名统一是 data4 的情况
# ------------------------------
load_score_data <- function(sample_name, tissue_type, treatment) {
  ...
  if (length(obj_names) == 0) stop("文件中没有对象: ", f)
  
  # 优先找 data4；否则取第一个 data.frame
  if ("data4" %in% obj_names) {
    ...
    if (length(candidate) == 0) stop("文件中未找到 data.frame: ", f)
...
# ======================================================================
# 6. 分析一：化疗前后肿瘤细胞 NuS / RiboSis 变化
# ======================================================================
# 这部分保留：在每种 tissue_type 内比较 Untreated vs Treated
...
# ======================================================================
# 7. 分析二：原发灶 vs 转移灶肿瘤细胞 NuS / RiboSis 变化
# ======================================================================
# 修改点：增加 treat 前后分别对比
# 即：
...
# ======================================================================
# 8. 分析三：原发灶 vs 转移灶的肿瘤细胞差异基因
# ======================================================================
# 修改点：untreated 和 treated 分别做 DE + GO

# ------------------------------
# 8.1 读取所有完整 Seurat 对象
# ------------------------------
