# GSE267499
setwd("/home/xxm_xxm/CJX_workspace/Dataset/GSE267499/")
rm(list = ls())
...
# 1. Read FPKM table
# ==============================
data <- read_excel("GSE267499_A375_fpkm.xlsx")
...
write.csv(sample_summary_raw, "GSE267499_sample_summary_rawFPKM.csv", row.names = FALSE)
...
write.csv(sample_summary_log, "GSE267499_sample_summary_logFPKM.csv", row.names = FALSE)

pdf("GSE267499_boxplot_logFPKM_used_for_DEG.pdf", width = 10, height = 6)
boxplot(
  exp_matrix,
  las = 2,
  outline = FALSE,
  main = "GSE267499 log2(FPKM+1) used for DEG"
)
dev.off()

pdf("GSE267499_density_logFPKM_used_for_DEG.pdf", width = 10, height = 6)
plotDensities(
  exp_matrix,
  main = "GSE267499 density plot"
)
dev.off()
...
write.table(
  ...
  file = "GSE267499_processing_decision.txt",
  sep = "\t",
  row.names = FALSE,
  quote = FALSE
)
...
write.csv(all_diff, "GSE267499_all_DEG.csv", row.names = FALSE)

write.csv(diffSig_fdr, "GSE267499_sig_DEG_FDR.csv", row.names = FALSE)
write.csv(up_fdr, "GSE267499_up_DEG_FDR.csv", row.names = FALSE)
write.csv(down_fdr, "GSE267499_down_DEG_FDR.csv", row.names = FALSE)

write.csv(diffSig_p, "GSE267499_sig_DEG_Pvalue.csv", row.names = FALSE)
write.csv(up_p, "GSE267499_up_DEG_Pvalue.csv", row.names = FALSE)
write.csv(down_p, "GSE267499_down_DEG_Pvalue.csv", row.names = FALSE)
