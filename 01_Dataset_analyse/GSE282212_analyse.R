# GSE282212
setwd("/home/xxm_xxm/CJX_workspace/Dataset/GSE282212/")
rm(list = ls())
...
# 1. Read TPM table
# ==============================
data <- read_excel("GSE282212_HCT116_24h_TPM.xlsx")
...
write.csv(sample_summary_raw, "GSE282212_sample_summary_rawTPM.csv", row.names = FALSE)

# ==============================
# 3. Optional: remove obvious structural RNAs
# Uncomment if you want to focus on coding/noncoding genes
...
write.csv(sample_summary_log, "GSE282212_sample_summary_logTPM.csv", row.names = FALSE)

pdf("GSE282212_boxplot_logTPM_used_for_DEG.pdf", width = 10, height = 6)
boxplot(
  exp_matrix,
  las = 2,
  outline = FALSE,
  main = "GSE282212 log2(TPM+1) used for DEG"
)
dev.off()

pdf("GSE282212_density_logTPM_used_for_DEG.pdf", width = 10, height = 6)
plotDensities(
  exp_matrix,
  main = "GSE282212 density plot"
)
dev.off()
...
write.table(
  ...
  file = "GSE282212_processing_decision.txt",
  sep = "\t",
  row.names = FALSE,
  quote = FALSE
)
...
write.csv(all_diff, "GSE282212_all_DEG.csv", row.names = FALSE)

write.csv(diffSig_fdr, "GSE282212_sig_DEG_FDR.csv", row.names = FALSE)
write.csv(up_fdr, "GSE282212_up_DEG_FDR.csv", row.names = FALSE)
write.csv(down_fdr, "GSE282212_down_DEG_FDR.csv", row.names = FALSE)

write.csv(diffSig_p, "GSE282212_sig_DEG_Pvalue.csv", row.names = FALSE)
write.csv(up_p, "GSE282212_up_DEG_Pvalue.csv", row.names = FALSE)
write.csv(down_p, "GSE282212_down_DEG_Pvalue.csv", row.names = FALSE)
