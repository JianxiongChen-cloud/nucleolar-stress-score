# Step4_liver manual annotation + RiboSis/NuS visualization and export (final version)
# ======================================================================
...
# Manual annotation mapping for each sample (from the original Step4)
...
for (name in sample_names) {
  ...
  if (!exists("obj_liver")) {
    warning("obj_liver was not found after load; sample ", name, " skipped.")
    next
  }
  
  cat("Object type:", class(obj_liver), "\n")
  cat("Default assay:", DefaultAssay(obj_liver), "\n")
  cat("Number of images:", length(obj_liver@images), "\n")
  ...
  if (!name %in% names(cluster_mapping_list)) {
    ...
    if (length(new.cluster.ids) != length(current_levels)) {
      warning("Sample ", name, " has a new.cluster.ids length that does not match levels(obj_liver); skipped RenameIdents.")
    } else {
  ...
  if (file.exists(ribosis_rdata)) {
    ...
    cat("Number of RiboSis genes present in obj_liver:", length(ribosis_genes_present), "\n")
  ...
  if (file.exists(nus_rdata)) {
    ...
    cat("Number of available UP genes in obj_liver:", length(up_present), "\n")
    cat("Number of available DOWN genes in obj_liver:", length(down_present), "\n")
