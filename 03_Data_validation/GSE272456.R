...
cat("Unique gene symbols:", length(unique(expr$GENE)), "\n")
cat("Duplicated gene symbols:", nrow(expr) - length(unique(expr$GENE)), "\n")

dup_genes <- expr$GENE[duplicated(expr$GENE)]
cat("Examples of duplicated gene symbols:\n")
print(head(dup_genes))

############################################################
## Step 2. Build expression matrix
## Add suffixes to duplicated gene symbols
## to avoid rowname conflicts
############################################################
...
## one-way ANOVA + Dunnett vs Control
############################################################
library(multcomp)
