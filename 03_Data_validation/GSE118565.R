...
############################################################
## 4. Construct group information
##    based on the actual sample names
############################################################
...
## Keep only samples defined in group_map
## and reorder according to group_map
...
############################################################
## 9. Convert expression matrix gene symbols to uppercase
...
############################################################
## 10. Calculate NuS scores using ssGSEA
############################################################
param <- ssgseaParam(
...
############################################################
## 3. Select target samples and reorder by group
...
##    Keep consistent with the previous DEG analysis:
##    TPM > 1 in at least 3 samples
############################################################
keep <- rowSums(expr_gene_sub > 1, na.rm = TRUE) >= 3
expr_gene_sub <- expr_gene_sub[keep, , drop = FALSE]
...
############################################################
## 7. Convert expression matrix gene symbols to uppercase
##    and intersect with the gene sets
############################################################
rownames(expr_gene_sub) <- toupper(rownames(expr_gene_sub))
...
############################################################
## 9. Statistical testing: Welch's t-test
############################################################
test_res <- data.frame(
