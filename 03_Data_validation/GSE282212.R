...
## 0. Path setup
############################################################
out_dir <- "/home/xxm_xxm/CJX_workspace/Data validation/GSE282212"
data_dir <- "/home/xxm_xxm/CJX_workspace/Dataset/GSE282212"
...
## 2. Read TPM table
############################################################
data <- read_excel(file.path(data_dir, "GSE282212_HCT116_24h_TPM.xlsx"))
...
############################################################
## 8. Convert gene symbols to uppercase and intersect with gene sets
############################################################
...
############################################################
## 9. Calculate NuS score by ssGSEA
############################################################
param <- ssgseaParam(
...
write.csv(scores_all, "GSE282212_NuS_scores_ssGSEA.csv", row.names = FALSE)

cat("===== NuS scores =====\n")
print(scores_all)

############################################################
## 10. Statistical test: Welch t-test
############################################################
test_res <- data.frame(
...
write.csv(test_res, "GSE282212_NuS_score_statistics_ttest.csv", row.names = FALSE)
print(test_res)
...
p <- ggplot(scores_all, aes(x = Group, y = Score, fill = Group)) +
  ...
  labs(
    title = "NuS ssGSEA scores in GSE282212",
    x = NULL,
    y = "NuS score"
  ) +
  theme_classic() +
...
ggsave("GSE282212_NuS_ssGSEA_violinplot.pdf", p, width = 4, height = 5, bg = "white")
ggsave("GSE282212_NuS_ssGSEA_violinplot.png", p, width = 4, height = 5, dpi = 300, bg = "white")
...
save(
  ...
  scores_all,
  test_res,
  file = "GSE282212_NuS_ssGSEA_workspace.Rdata"
)
