...
## expression matrix construction + ssGSEA scoring + visualization
############################################################
...
############################################################
## Step 2. Read platform annotation
############################################################
ids_raw <- read_tsv(
  "GPL15207-17536.txt",
  comment = "#",
...
############################################################
## Step 3. Construct probe-level expression matrix
############################################################
data_raw <- data_raw %>%
  mutate(ID_REF = str_replace_all(`"ID_REF"`, '"', ""))

## Keep only valid GSM sample columns
...
############################################################
## Step 4. Retain annotated probes
## and collapse duplicated probes
############################################################
...
## Sort by symbol + median expression
## and retain the probe with the highest expression
...
############################################################
## Step 6. Construct group information
## based on actual sample names
############################################################
...
## Select the core gene set here
geneSets_use <- geneSets_all
geneset_label <- "all"

############################################################
## Step 8. Intersect with the expression matrix
############################################################
...
## Bar plot + standard error
## Use the fixed light green-cyan-blue color palette
############################################################
summary_data <- scores %>%
  group_by(Group) %>%
  summarise(
