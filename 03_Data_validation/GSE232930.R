...
## ssGSEA scoring + statistical analysis + visualization
## Full gene set only
############################################################
...
geo_id <- "GSE232930"
expr_raw_file <- "GSE232930_Primed_H9_hL1KD_log2Nom_counts_cutoff_0.5_BatchCo.txt.gz"
...
############################################################
############################################################
## Step 1. Read and process expression matrix
############################################################
...
# Extract gene symbols before the first "-"
raw_id <- rownames(dat)
clean_id <- stringr::str_remove(raw_id, "-.*")
clean_id <- toupper(trimws(clean_id))
...
rownames(expr_numeric) <- raw_id
colnames(expr_numeric) <- colnames(dat)

# Calculate median expression for each row
expr_info <- expr_numeric
expr_info$median <- apply(expr_numeric, 1, median, na.rm = TRUE)
expr_info$symbol <- clean_id

# Remove empty gene symbols
expr_info <- expr_info[!is.na(expr_info$symbol) & expr_info$symbol != "", ]

# For duplicated gene symbols, keep the row with the highest median expression
expr_info <- expr_info[order(expr_info$symbol, -expr_info$median), ]
expr_info <- expr_info[!duplicated(expr_info$symbol), ]

# Build gene-level expression matrix
expr.mat <- as.matrix(expr_info[, colnames(expr_numeric), drop = FALSE])
rownames(expr.mat) <- expr_info$symbol
...
# Remove genes containing NA values if present
expr.mat <- expr.mat[rowSums(is.na(expr.mat)) == 0, , drop = FALSE]

# Save processed expression matrix
save(expr.mat, file = "GSE232930_processed_expression_matrix.Rdata")
write.csv(expr.mat, file = "expressionmetrix_GSE.csv")
############################################################
## Step 2. Group information
############################################################
## If actual sample names are known,
## replace them directly with the real sample names
...
## If group_info$Sample does not match actual column names,
## automatically replace with expression matrix column names
if (!all(group_info$Sample %in% colnames(expr.mat))) {
  if (nrow(group_info) == ncol(expr.mat)) {
    group_info$Sample <- colnames(expr.mat)
    cat("group_info$Sample Automatically replaced with the column names of the expression matrix.\n")
  } else {
    stop("group_info The number of samples is inconsistent with the number of columns in the expression matrix. Please check.")
  }
}
...
############################################################
## Step 3. Read the complete gene set
############################################################
load(geneset_file)
...
if (!all(c("NuS_Up", "NuS_Down") %in% rownames(ssgsea.res))) {
  stop("ssGSEA NuS_Up or NuS_Down not found in ssGSEA results")
}
...
## Each treatment group versus control
...
## Overall comparison (violin plot style)
############################################################
fill_colors <- c(
  "Control" = "#C4E7C1",
  "so" = "#93D4BC",
...
## Comparison versus control group (violin plot style)
############################################################
