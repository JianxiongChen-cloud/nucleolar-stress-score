...
## 0. 路径设置
############################################################
out_dir <- "/home/xxm_xxm/CJX_workspace/Data validation/GSE267499"
data_dir <- "/home/xxm_xxm/CJX_workspace/Dataset/GSE267499"
...
## 2. 读取FPKM表
############################################################
data <- read_excel(file.path(data_dir, "GSE267499_A375_fpkm.xlsx"))
...
############################################################
## 3. 选择表达列
##    与DEG代码一致：先6h_rep1-3，再0h_rep1-3
############################################################
...
write.csv(scores_all, "GSE267499_NuS_scores_ssGSEA.csv", row.names = FALSE)
...
write.csv(test_res, "GSE267499_NuS_score_statistics_ttest.csv", row.names = FALSE)
print(test_res)
...
p <- ggplot(scores_all, aes(x = Group, y = Score, fill = Group)) +
  ...
  labs(
    title = "NuS ssGSEA scores in GSE267499",
    x = NULL,
    y = "NuS score"
  ) +
  theme_classic() +
...
ggsave("GSE267499_NuS_ssGSEA_violinplot.pdf", p, width = 4, height = 5, bg = "white")
ggsave("GSE267499_NuS_ssGSEA_violinplot.png", p, width = 4, height = 5, dpi = 300, bg = "white")
...
save(
  ...
  scores_all,
  test_res,
  file = "GSE267499_NuS_ssGSEA_workspace.Rdata"
)