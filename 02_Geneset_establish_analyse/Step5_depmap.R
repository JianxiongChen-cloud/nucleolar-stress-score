...
#===============================
# 1. Read NuStress_UP / NuStressCore_UP
#===============================
...
# Modify here if your file names are different
...
#===============================
# 3. Clean DepMap column names
# Your format is: A1BG (1), A1CF (29974), ...
#===============================
extract_gene_symbol <- function(x) {
  x <- gsub("\\s*\\([^\\)]*\\)$", "", x)   # Remove trailing parenthesized IDs
  x <- trimws(toupper(x))
  x
}
...
#===============================
# 5. Aggregate dependency score for each gene
# Default: median; change aggregate_fun to "mean" if needed
#===============================
...
#===============================
# 6. Plotting function: match the reference figure as closely as possible
#===============================
make_dep_density_plot <- function(plot_df_one, essential_percent, set_title,
  ...
  # Consider both histogram and density maximum heights to avoid clipping bars or curves
  ...
  # Add sufficient top margin to ensure complete display
  ymax <- ymax_raw * 1.30
...
#===============================
# 7. Auxiliary plot: essential gene proportion
#===============================
