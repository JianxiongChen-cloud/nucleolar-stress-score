...
# Step 2 output directory (based on your Step 2 naming convention)
...
#===============================
# 3. Unified plotting functions
#    Color palette strictly follows the reference code
#===============================
...
#===============================
# 4. KEGG analysis function for a single gene set
#===============================
run_kegg <- function(gene_symbols, set_name, out_dir) {
  ...
  cat("Successfully mapped ENTREZIDs::", length(entrez_ids), "/", length(gene_symbols), "\n")

  if (length(entrez_ids) < 5) {
    cat("Too few ENTREZIDs for enrichment analysis, skipping:", set_name, "\n\n")
    return(NULL)
  }
  ...
  if (is.null(kegg_result) || nrow(as.data.frame(kegg_result)) == 0) {
    cat("No significantly enriched KEGG pathways found:", set_name, "\n\n")
    return(NULL)
  }
  ...
  cat("Identified", nrow(result_df), "significantly enriched KEGG pathways\n")
  ...
  cat("\n=== Top 10 most significant KEGG pathways ===\n")
...
#===============================
# 5. Perform KEGG analysis for NuStress and NuStressCore separately
#===============================
